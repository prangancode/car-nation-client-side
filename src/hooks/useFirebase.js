...
const useFirebase = () => {
  ...
  const saveUserToDatabase = (email, displayName, method) => {
    ...
    const url = `https://car-nation-server-side-production.up.railway.app/users`;
    axios({
      method: method,
      url: url,
      data: user,
  ...
  // check is admin
  useEffect(() => {
    const url = `https://car-nation-server-side-production.up.railway.app/users/${user?.email}`;
