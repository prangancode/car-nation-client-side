...
const DashboardHome = () => {
	...
	// load orders
	useEffect(() => {
		let url = `https://guarded-beach-83428.herokuapp.com/orders?email=${user.email}`;
		if (isAdmin) {
			url = `https://guarded-beach-83428.herokuapp.com/orders`;
		}
		axios
			.get(url)
			.then(res => {
	...
	// load reviews
	useEffect(() => {
		const url = `https://guarded-beach-83428.herokuapp.com/reviews`;
		axios
			.get(url)
			.then(res => {
