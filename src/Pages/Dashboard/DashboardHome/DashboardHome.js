...
const DashboardHome = () => {
  ...
  // load orders
  useEffect(() => {
    let url = `https://car-nation-server-side-production.up.railway.app/orders?email=${user.email}`;
    if (isAdmin) {
      url = `https://car-nation-server-side-production.up.railway.app/orders`;
    }
    axios
      .get(url)
      .then((res) => {
  ...
  // load reviews
  useEffect(() => {
    const url = `https://car-nation-server-side-production.up.railway.app/reviews`;
