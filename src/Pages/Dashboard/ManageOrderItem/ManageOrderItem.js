...
const ManageOrderItem = ({ order, orders, setOrders, index }) => {
  ...
  const onSubmit = (data) => {
    ...
    const url = `https://car-nation-server-side-production.up.railway.app/orders/${order._id}`;
    axios
  ...
  const handleRemoveOrder = (id) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        const url = `https://car-nation-server-side-production.up.railway.app/orders/${id}`;
        axios
          .delete(url)
          .then((res) => {
            console.log(res);
